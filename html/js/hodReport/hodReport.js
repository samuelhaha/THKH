...
$(document).ready(function () {
        ...
        $("#submitBtn").click(function (e) {
            e.preventDefault();
            var k_hod_comments = $("#k_hod_comments").val();
            var hpo_id = $("#selectedHpo").val();
            var form_data = new FormData();
            form_data.append('k_hod_comments',k_hod_comments);
            form_data.append('hpo_id',hpo_id);
    
            if (gethorNum.has('horNum')) {
                horNum = gethorNum.get('horNum');
                $.ajax({
                    method: "POST",
                    url: "/THKH/laravel/api/hod-add/" + horNum,
                    headers: { Authorization: 'Bearer ' + sessionStorage.getItem("jwt") },
                    data: form_data,
                    dataType: 'json',
